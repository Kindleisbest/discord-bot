...
const PAGES: Page[] = ['overview', 'messages', 'inbox', 'events', 'permissions', 'setup', 'accessibility', 'privacy'];
...
export function App() {
  ...
  useEffect(() => { document.title = `${page === 'overview' ? 'Overview' : page === 'messages' ? 'Messages' : page === 'inbox' ? 'Staff inbox' : page === 'events' ? 'Events' : page === 'permissions' ? 'Permissions' : page === 'setup' ? 'Setup guide' : page === 'accessibility' ? 'Accessibility statement' : 'Privacy & retention'} — Discord Bot`; }, [page]);
