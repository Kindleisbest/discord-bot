...
const ACTION_LABELS: Record<string, string> = { 'permissions.updated': 'Website permissions updated', 'permissions.update': 'Website permissions updated', 'auth.login': 'Administrator signed in', 'onboarding.completed': 'Administrator tutorial completed' };
...
export function Overview({ detail, onAccessError }: { detail: GuildDetail; onAccessError: (error: ApiError) => void }) {
  ...
  return <>
    ...
    <section className="activity-section" aria-labelledby="activity-title"><div className="section-heading"><h2 id="activity-title">Latest activity</h2>{allowed ? <button className="button button-outline button-small" onClick={() => setRevision(value => value + 1)} disabled={activity === null && !error}><RefreshCw size={17} aria-hidden="true" />Refresh</button> : null}</div>
      {!allowed ? <p>You do not have access to view activity. Contact your server owner.</p> : error ? <ErrorNotice message={error} retry={() => setRevision(value => value + 1)} /> : activity === null ? <Loading>Loading server activity…</Loading> : activity.length === 0 ? <div className="empty-state"><ActivityIcon size={34} aria-hidden="true" /><h3>No activity yet</h3><p>Website permission changes will appear here as your team sets up this server.</p></div> : <ol className="activity-list">{activity.map(item => <li key={item.id}><div><h3>{ACTION_LABELS[item.action] ?? item.action.replace(/[._]/g, ' ')}</h3><p>Discord user <code>{item.actorId}</code>{item.targetId ? <> · Target <code>{item.targetId}</code></> : null}</p></div><time dateTime={new Date(item.createdAt).toISOString()}>{new Date(item.createdAt).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}</time></li>)}</ol>}
    </section>
    <p className="supporting-note">This foundation records website administration activity. Member message tools will arrive in the next stages.</p>
  </>;
}
