...
**Latest resume point: encrypted member XP storage and atomic award/lifecycle safeguards implemented and tested. Next: leveling services and the validated settings editor, then live membership/activity integration.** See the final milestone below and docs/LEVELING_CHECKPOINT.md; earlier checkpoint entries are history. Confirmed leveling answers are recorded in docs/LEVELING_QUESTIONNAIRE.md; polls/RSVPs are deferred for a later discussion. Live Discord/Pi deployment and the final PDF remain later.

**Resumed 2026-09-21:** Andrew withdrew the mistaken “got it working” message; there is still no confirmed live Discord/Pi installation. Live usage reporting is available again: at this resume, 93% five-hour and 99% weekly remained, with two reset credits available and none redeemed. The earlier save-only stop has been superseded by the explicit resume request. Continue the remaining leveling questions from the saved checkpoint without repeating confirmed choices.
...
6. [ ] Finish detailed leveling choices; implement approved pieces in bounded stages. Poll votes and event RSVPs are deferred for a later discussion.
...
## Leveling progression checkpoint — 2026-09-21

The explicit resume request supersedes the earlier save-only stop. The mistaken “got it working” statement was withdrawn; live Discord and Pi deployment remain unverified. Usage reporting works again. The latest check before saving showed **49% five-hour and 92% weekly remaining**; this is a historical reading, not a promise of current allowance. Two reset credits remain available; none were redeemed. Credits remain restricted to finishing saves after ordinary allowances run out.

Latest choices supersede earlier conflicting questionnaire answers: **no daily XP cap**, reaction rewards only on source messages up to **30 days old**, **all-time leaderboard only**, all new human messages including replies/emoji/links/attachments, no XP for edits, and a forum opening counts once. Three separate website grants cover viewing records, adjusting member XP, and managing settings/rewards, while preserving the existing Discord Administrator/owner gate and higher-role delegation. Reward roles update automatically after XP reductions or milestone changes, managing only roles this bot assigned. The finished feature should **start enabled with the agreed defaults** in each server. Poll votes and event RSVPs are explicitly deferred: ask Andrew again later and do not implement them now. Expected server/member scale is unknown; do not claim a tested capacity.

Implemented only the independent progression module in `shared/leveling.ts`: exact bigint calculations for `10*n²` per advance, cumulative thresholds, logarithmic XP-to-level lookup with no fixed cap, and remaining-progress values. Six focused tests verify approved milestones, threshold boundaries, huge integer values, inversion, and invalid inputs. **All 255 tests, both typechecks, and production build pass.** No website behavior changed, so the previous simulated browser checks remain the latest UI validation. No leveling collector, intent, database table, command, reward delivery, or live activation is added by this stage.

NEXT: build per-server leveling settings and three separate permission grants, then durable exact XP and member lifecycle storage with atomic shared cooldowns, opt-out/departure retention, and bounded deduplication. Follow the staged sequence in docs/LEVELING_QUESTIONNAIRE.md. Keep source message bodies out of leveling storage and audit work on its separate branch. Save to GitHub main with a verified non-force update. Deployment hardening, Pi/Discord validation, the free-address/Cloudflare decision, Pi Party, and final illustrated PDF remain outstanding.

## Leveling settings persistence checkpoint — 2026-09-21

Implemented encrypted per-server leveling settings with enabled/10 XP/30-second/no-cap defaults, exclusions, exact milestone-level strings, and announcement preferences. Saves require bounded strict input, an expected revision, and a reason. Settings, encrypted reason history, and ordinary administration activity commit together or roll back together. Corrupt records fail closed. Reasons expire at 90 days; configuration persists. Runtime startup/hourly cleanup/guild removal include the settings store. No settings HTTP route, role/channel discovery, editor, XP record, collector, new intent, or reward delivery has been added.

The existing permission editor now supports independent `leveling.view`, `leveling.adjust`, and `leveling.manage` grants. Existing login AND Administrator/owner checks, role hierarchy, protected grants, server isolation, and CSRF/origin checks remain. A leveling grant does not imply owner status or permission to reset everyone. **269 automated tests, both typechecks, and production build pass.** See docs/LEVELING_CHECKPOINT.md for contract bounds and remaining work. The three added checkbox labels use the existing UI; no new browser verification is claimed.

The latest allowance check before final verification/saving was **11% five-hour and 86% weekly remaining**. No reset credit was redeemed. Finish saving this verified checkpoint and stop before more feature work; credits remain reserved only for finishing saves. On the next resume, check live usage again. NEXT: durable member XP/lifecycle storage, then validated settings service/routes/editor and approved earning sources. Polls/RSVPs remain deferred; audit remains on its separate branch; live installation remains unconfirmed.

## Leveling member storage checkpoint — 2026-09-22

Completed the interrupted storage stage. Member XP is encrypted and exact; award calculations read current server settings and atomically save progress plus a hashed, authenticated duplicate receipt. Message/reaction/voice sources share one cooldown and have no daily cap. Freshness, enrollment, reaction-age, capacity, and clock-regression guards fail closed. Cooldown-rejected events are consumed to prevent delayed replay. Guild/source event identity cannot be moved to a different recipient.

Explicit enrollment is required. Departure blocks earning and retains XP for 30 days without extending the deadline on repeated notifications. Expired progress is hidden immediately and removed by cleanup. Rejoining before expiry preserves XP; after expiry it starts fresh. Opt-out clears XP/cooldown data and keeps a minimal preference plus temporary duplicate receipts. Explicit opt-in starts fresh; ordinary joins preserve opt-out. No Discord role action is implemented, so the complete member-facing opt-out flow is still future work. Corrupt records fail closed, including expiry metadata before reads/pruning; cleanup uses bounded batches. Runtime startup/hourly cleanup/guild removal include member storage.

**285 automated tests, both typechecks, and production build pass.** Sixteen new tests cover exact huge values, privacy/lifecycle boundaries, encrypted disk persistence, duplicate and cooldown rules, no daily cap, capacity, corruption, clock regression, and rollback after both initial and partially completed writes. No frontend code or live Discord feature was added; no new browser validation is claimed. Capacity bounds are safeguards, not measured Pi support.

NEXT: settings/member service authorization and the settings editor; current Discord role/channel validation; safe live membership reconciliation including missed offline departures; approved activity collectors; administrative XP changes and owner-only reset; leaderboard, role reconciliation, announcements, and private commands. Polls/RSVPs remain deferred. Audit stays separate, and live Discord/Pi tests, hardening/free address, Pi Party, and illustrated installation PDF remain later. Latest pre-save allowance reading: **14% five-hour and 71% weekly remaining**. Save and verify this bounded checkpoint before stopping; no reset credits redeemed.
