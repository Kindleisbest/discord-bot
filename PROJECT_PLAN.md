...
**Latest resume point: leveling choices consolidated and exact progression calculations implemented/tested. Next: per-server settings, permissions, and durable XP storage.** See the final milestone below; earlier checkpoint entries are history. Confirmed leveling answers are recorded in docs/LEVELING_QUESTIONNAIRE.md; polls/RSVPs are deferred for a later discussion. Live Discord/Pi deployment and the final PDF remain later.
