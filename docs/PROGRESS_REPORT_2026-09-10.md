...
| Leveling system | Calculations, settings/member storage, and grants implemented; feature incomplete | Exact progression, encrypted settings/XP, atomic shared cooldown and duplicate prevention, opt-out preferences, and 30-day departure retention. Defaults are enabled when shipped, 10 XP, 30 seconds, and no daily cap. Live collection/membership handling, member administration, rewards, commands, leaderboard, and settings editor remain. Polls/RSVPs are deferred. |
...
- The latest implementation passed **285 automated security and behavior tests**, both application type checks, and the production build on 22 September 2026. Leveling coverage includes exact progression, encrypted settings/member records, shared cooldowns, duplicate prevention, opt-out/rejoin rules, strict validation, rollback, retention, disk persistence, capacity, and protected permission delegation.
