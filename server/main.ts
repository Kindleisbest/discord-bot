...
const bot=createBot(config,{addActivity:(...args)=>store.addActivity(...args),removeGuild:id=>{tutorialStore.removeGuild(id);eventStore.removeGuild(id);inboxStore.removeGuild(id);store.removeGuild(id);}});
...
const app=await buildApp(config,store,new DiscordHttpApi(config),bot,inbox,events,tutorials);
